...
test "parser: simple field" {
    ...
    const file = try parser.parseSource(a, src, "test.skg");
...
test "parser: float field" {
    ...
    const file = try parser.parseSource(arena.allocator(), src, "test.skg");
...
test "parser: skg_version and schema_version" {
    ...
    try testing.expectApproxEqAbs(1.0, file.skg_version.?, 0.001);
...
test "parser: mixed int float rejected in array" {
    ...
    const result = parser.parseSource(arena.allocator(), src, "test.skg");
...
test "merge: overlay field overwrites base" {
    ...
    const base = try parser.parseSource(a, "accent: \"green\"", "base.skg");
    const overlay = try parser.parseSource(a, "accent: \"purple\"", "overlay.skg");
